...
export const TOOLS = [
  ...
  },
  {
    ...
    description: 'Opgør Gustavs PENGEFORBRUG med rigtige tal fra hans bankdata, pr. måned. Brug når han spørger hvad han BRUGER/har brugt af penge: "hvor mange penge bruger jeg på mad", "hvad bruger jeg pr. måned på dagligvarer", "hvor meget har jeg brugt på takeaway/sodavand", "hvad koster transport mig", "hvad bruger jeg penge på". Regner deterministisk på posteringerne (kategori/synd pr. måned). Et penge/forbrugs-spørgsmål er ALDRIG search_memory og ALDRIG save_note.',
    input_schema: {
      type: 'object',
      properties: {
        focus: { type: 'string', description: 'Hvad forbruget skal opgøres for, med Gustavs egne ord: "mad", "dagligvarer", "takeaway", "transport", "sodavand"... Dækker spørgsmålet FLERE dele på én gang ("dagligvarer og mad ude/takeaway"), så send HELE frasen ordret - vælg aldrig kun den ene del. Udelad for et samlet overblik over alle udgifter.' },
        months: { type: 'integer', description: 'Hvor mange måneder tilbage (1-12). Udelad for standard (de seneste 3).' },
      },
      required: [],
    },
  },
...
function buildStableSystem(globalFacts = '') {
  ...
  return [
    ...
    '- Vil Gustav TILPASSE/SKALERE en kendt ret til sit faktiske indkøb ("tilpas den bagte ret, jeg har 800g oksekød 12%", "skalér hønsesuppen til 6 portioner", "ret den til med 1 kg kylling") -> suggest_meal med constraints der indeholder BÅDE rettens navn OG mængderne. Det er ikke search_memory.',
