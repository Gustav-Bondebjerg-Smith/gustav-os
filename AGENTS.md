...
- JS-regex `\b` matcher IKKE æ/ø/å (kun ASCII tæller som word-tegn). En frase som "starter på" får derfor aldrig en word-boundary efter "på" og matchede aldrig. Brug Unicode-grænser `(?<![\p{L}\p{N}]) ... (?![\p{L}\p{N}])` med `u`-flag i stedet (se `ACTIVITY_TRIGGER` i `lib/telegram-activity.ts`). Gælder alle danske trigger-regexes.
